...
def test_settings_reads_database_env(monkeypatch: pytest.MonkeyPatch):
    import secrets
    db_pass = f"mock-pass-{secrets.token_hex(8)}"
    monkeypatch.setenv("SOUNDBOX_DATABASE_HOST", "db.example.com")
    monkeypatch.setenv("SOUNDBOX_DATABASE_PORT", "5433")
    monkeypatch.setenv("SOUNDBOX_DATABASE_USERNAME", "appuser")
    monkeypatch.setenv("SOUNDBOX_DATABASE_PASSWORD", db_pass)
    monkeypatch.setenv("SOUNDBOX_DATABASE_NAME", "soundbox")

    cfg = Settings()
    assert cfg.database_host == "db.example.com"
    assert cfg.database_port == 5433
    assert cfg.database_username == "appuser"
    assert cfg.database_password == db_pass
...
def test_require_database_raises_when_missing(monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setenv("SOUNDBOX_DATABASE_HOST", "")
    cfg = Settings()
